<template>
  <div>
    ...
    <h1 class="mb-8 font-bold text-3xl">
      <inertia-link class="text-indigo-400 hover:text-indigo-600" :href="route('discussions.index', discussion.course_module_id)">Diskusi</inertia-link> / {{ discussion.title }}
    </h1>
    ...
    <div v-if="userLogin.usertype_id != 2" class="mb-10">
      <inertia-link class="btn-indigo" :href="route('discussions.change_status', discussion.id)">
        <span>{{ discussion.is_active ? 'Tutup' : 'Buka' }}</span>
    ...
    <div class="max-w rounded mb-5">
      <div class="mb-5">
        ...
        <div v-else class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
          ...
          <span class="block sm:inline">Diskusi telah ditutup.</span>
        </div>
      </div>
        ...
        <div class="flex items-start px-4 py-6">
          ...
          <div class="">
            ...
            >
             ...
             <a :href="route('discussions.reply', comment.id)" class="flex-1 mx-1">
             <button  v-if="userLogin.usertype_id == 3 || userLogin.usertype_id == 1" 
               class="text-green-500 bg-transparent border border-solid border-green-500 hover:bg-green-500 hover:text-white active:bg-green-600 font-bold uppercase text-xs px-4 py-2 rounded-full outline-none focus:outline-none mr-1 mb-1" type="button" style="transition: all .15s ease">
             Balas</button> </a>
          </div>
        </div> 
    ...
    <div class="max-w rounded mb-5 mt-10">
          ...
          <div class="flex flex-wrap -mx-3 mb-6">
            ...
            <div class="w-full md:w-full px-3 mb-2 mt-2">
              <textarea
                ...
                class="bg-gray-100 rounded border border-gray-400 leading-normal resize-none w-full h-20 py-2 px-3 font-medium placeholder-gray-700 focus:outline-none focus:bg-white" name="body" placeholder="Ketik komentar disini" required
              />
            </div>
            <div class="w-full md:w-full flex items-start md:w-full px-3">
              ...
              <div class="-mr-1">
                <input type="submit" class="bg-white text-gray-700 font-medium py-1 px-4 border border-gray-400 rounded-lg tracking-wide mr-1 hover:bg-gray-100" value="Posting Komentar">
              </div>
            </div>
          </div>
        </form>
      ...
      <div v-else class="bg-white rounded shadow overflow-hidden max-w">
        <div class="bg-blue-100 border-t border-b border-blue-500 text-blue-700 px-4 py-3" role="alert">
          <h3 class="font-bold">Diskusi ini telah ditutup</h3>
          <p class="text-sm">Kamu tidak bisa memberikan komentar jika diskusi telah ditutup. Tetapi kamu masih bisa melihat komentar yang sudah ada sebelumnnya.</p>
        </div>
      </div>
    </div>
  </div>
  ...
  data() {
    return {
      ...
      form: {
        ...
        fullname:this.discussion.fullname
      },
    }
  },
  methods: {
    ...
    destroy(id) {
      if (confirm('Apakah kamu yakin ingin menghapus ?' + id)) {
        this.$inertia.delete(
